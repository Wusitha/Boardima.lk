    ...
    public ResponseEntity<String> authenticateUser(@RequestBody LoginDTO loginDTO, final HttpServletRequest request)
    {
        try{
            ...
            if(authentication.isAuthenticated())
            {
                ...
                return  new ResponseEntity<>((authentication1.getAuthorities().toString()).substring(6,7)+ RequestContextHolder.currentRequestAttributes().getSessionId(), HttpStatus.OK);
